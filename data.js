export const REVIEWS = [
  {
    username: "jennifer",
    content:
      "I'm loving this product! It exceeded my expectations. Highly recommended.",
    rate: 5,
  },
  {
    username: "alex",
    content: "Not bad, but could be better. It's just okay.",
    rate: 3,
  },
  {
    username: "sarah",
    content: "Absolutely fantastic! This product is a game-changer.",
    rate: 5,
  },
  {
    username: "john",
    content: "Disappointed with the quality. It didn't meet my expectations.",
    rate: 3,
  },
  {
    username: "emily",
    content: "Great value for the price. I'm very satisfied with my purchase.",
    rate: 4,
  },
  {
    username: "david",
    content:
      "Poor customer service. The company needs to improve their support.",
    rate: 2,
  },
  {
    username: "lisa",
    content: "This product is a lifesaver. It solved my problem instantly.",
    rate: 5,
  },
  {
    username: "michael",
    content: "Not impressed. The product didn't live up to the hype.",
    rate: 3,
  },
  {
    username: "natalie",
    content: "I can't imagine my life without this product. It's amazing!",
    rate: 5,
  },
  {
    username: "peter",
    content: "Average product. Nothing special about it.",
    rate: 3,
  },
  {
    username: "olivia",
    content:
      "The best product I've ever used. I'm blown away by its performance.",
    rate: 5,
  },
  {
    username: "william",
    content: "Waste of money. I regret buying this product.",
    rate: 2,
  },
  {
    username: "samantha",
    content: "Highly disappointed. The product arrived damaged.",
    rate: 2,
  },
  {
    username: "james",
    content: "Decent product, but there are better alternatives available.",
    rate: 3,
  },
  {
    username: "emma",
    content: "I'm in love with this product. It exceeded my expectations.",
    rate: 5,
  },
  {
    username: "ryan",
    content: "Not worth the price. I expected better quality.",
    rate: 2,
  },
  {
    username: "ava",
    content: "This product is a game-changer. I can't recommend it enough.",
    rate: 5,
  },
  {
    username: "matthew",
    content: "The product is good, but the customer service needs improvement.",
    rate: 4,
  },
  {
    username: "mia",
    content:
      "I'm extremely satisfied with this product. It's worth every penny.",
    rate: 5,
  },
  {
    username: "ethan",
    content: "Below average. It didn't meet my expectations.",
    rate: 3,
  },
  {
    username: "chloe",
    content: "This product is a disappointment. I regret buying it.",
    rate: 2,
  },
  {
    username: "daniel",
    content: "Impressive product with excellent quality. Highly recommended.",
    rate: 5,
  },
  {
    username: "grace",
    content:
      "The product is average. It does the job, but nothing extraordinary.",
    rate: 3,
  },
  {
    username: "jackson",
    content:
      "This product is a complete waste of money. I'm highly dissatisfied.",
    rate: 1,
  },
  {
    username: "harper",
    content:
      "I'm pleasantly surprised by this product. It exceeded my expectations.",
    rate: 4,
  },
  {
    username: "aiden",
    content: "Not impressed. The product didn't live up to the hype.",
    rate: 3,
  },
  {
    username: "sophia",
    content: "This product is amazing. It's a game-changer for me.",
    rate: 5,
  },
  {
    username: "logan",
    content: "I expected better. The product didn't deliver as promised.",
    rate: 3,
  },
  {
    username: "abigail",
    content:
      "I'm extremely satisfied with this product. It's worth every penny.",
    rate: 5,
  },
  {
    username: "lucas",
    content: "Disappointing product. It didn't meet my expectations.",
    rate: 2,
  },
];

export const DESCRIPTIONS = {
  adidas:
    "The Adidas Ultraboost shoes offer an incredibly comfortable fit, with their cushioned sole and breathable upper. The only downside was that the laces were a bit too long and required double knotting, but overall, these shoes are a top-notch choice for running enthusiasts.",
  converse:
    "The Converse Chuck Taylor All Stars are timeless classics that never disappoint. They provide a snug and comfortable fit, and their sleek design goes well with any outfit. The only minor issue was that the rubber sole tends to wear out quickly, but the overall quality and style make them a must-have in any sneaker collection.",
  nike: "The Nike React Element 55 sneakers are the epitome of style and comfort. The lightweight construction and responsive cushioning make them perfect for all-day wear. The only drawback was that the sizing runs a bit small, so it's advisable to go half a size up. Nevertheless, these shoes are a game-changer in terms of both looks and performance.",
  puma: "The Puma Suede Classic shoes are a true embodiment of retro coolness. They offer a comfortable fit with their padded collar and soft suede upper. The only downside was that the outsole tends to be a bit slippery on wet surfaces, but the iconic design and versatility of these sneakers make them a go-to choice for casual and streetwear fashion.",
  vans: "The Vans Old Skool sneakers are a timeless icon in the world of skateboarding and street fashion. Their sturdy canvas upper and signature side stripe provide durability and style. The only minor issue was that the insole could use a bit more arch support for long hours of wear, but overall, these shoes are a classic staple that never goes out of style.",
};

export const TITLES = {
  nike: "Nike Air Max React",
  adidas: "Adidas New Hero Max",
  converse: "Converse Chuck Taylor All Star",
  puma: "Puma Shox Vulcan",
  vans: "Vans Timu Lara",
};
export const IMGS = [
  "C:/Users/zeyad/OneDrive/Desktop/E-Comm/src/assets/img/products/product-1.png",
  "C:/Users/zeyad/OneDrive/Desktop/E-Comm/src/assets/img/products/floating-product-2.png",
  "C:/Users/zeyad/OneDrive/Desktop/E-Comm/src/assets/img/products/floating-product-3.png",
  "C:/Users/zeyad/OneDrive/Desktop/E-Comm/src/assets/img/products/product-5.png",
  "C:/Users/zeyad/OneDrive/Desktop/E-Comm/src/assets/img/products/product-6.png",
  "C:/Users/zeyad/OneDrive/Desktop/E-Comm/src/assets/img/products/product-7.png",
  "C:/Users/zeyad/OneDrive/Desktop/E-Comm/src/assets/img/products/product-9.png",
  "C:/Users/zeyad/OneDrive/Desktop/E-Comm/src/assets/img/products/floating-product-1.webp",
];
export const BRANDS = ["Adidas", "Converse", "Nike", "Puma", "Vans"];
export const CATEGORIES = ["Shoes", "Belts", "Bags"];
export const COLORS = [
  "red",
  "green",
  "blue",
  "black",
  "yellow",
  "violet",
  "white",
];

export const SIZES = ["SM", "MD", "LG", "XL", "2XL"];
